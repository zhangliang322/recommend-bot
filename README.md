...
Windows 下直接双击仓库根目录的 `启动桌面运营台.cmd`，无需打开浏览器。
...
桌面版提供候选搜索和类目筛选、商品详情与卡片、公域/私域文案复制、单品及批量
审核、数据源启停、多多进宝连接测试和同步日志。运行数据只保存在本机。
...
网页管理端不再是首选呈现方式，仅保留给接口调试和兼容场景。安装项目后执行
`reco-admin`，接口调试页位于 `http://127.0.0.1:8765/docs`。
